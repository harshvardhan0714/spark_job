...
def model_training(spark):
    try:     
        ...
        # result.show()
        # df.show()
        # df.select(countDistinct('sensor_id')).show()
        # grouped_df = df.groupBy("sensor_id")
        # grouped_data = grouped_df.collect()

        # for group in grouped_data:
        #     print(f"Sensor ID: {group[0]}")  # Access group key (sensor_id)
        #     for row in group[1].toLocalIterator():
        #         print(f"\tSensor Name: {row.sensor_name}, Value: {row.value}")

        # for i in df_grouped:
        #     i.show()
        # train, test = train_test_split(df)
        # # print("train_data:",train.count(),"test_data:",test.count())
        # window = Window.partitionBy("sensor_id").orderBy("creation_time").rangeBetween(Window.currentRow, 1)



        # grouped_df = df.groupBy("sensor_id")

        # # Apply the split function to each group and collect the results
        # train_test_splits = grouped_df.applyInPandas(train_test_split, schema=df.schema)

        exit()
        
        """############################################"""
        # Defined features and target
        features = ['temperature_2m', 'relative_humidity_2m', 'apparent_temperature', 'precipitation', 'wind_speed_10m', 'wind_speed_100m', 'creation_time', 'sensor_id', 'count', 'is_holiday', 'lag1', 'lag2', 'lag3', 'lag4', 'lag5', 'lag6', 'lag7', 'day', 'hour', 'month', 'dayofweek', 'quarter', 'dayofyear', 'weekofyear', 'year']
        target = "consumed_unit"

        # Feature selection
        X_train = df.select(features)
        y_train = df.select(target)

        # XGBoost model definition
        xgb_model = XGBRegressor(featuresCol="features", labelCol="label")

        # Hyperparameter grid definition
        config= ...
        param_grid = ParamGridBuilder() \
        .addGrid(xgb_model.nEstimators, config.get("n_estimators")) \
        .addGrid(xgb_model.maxDepth, config.get("max_depth")) \
        .addGrid(xgb_model.learningRate, config.get("learning_rate")) \
        .build()

        # Train-validation split and hyperparameter tuning
        trainval_split = TrainValidationSplit(estimator=xgb_model,
                                            evaluator=RegressionEvaluator(metricName="rmse"),
                                            estimatorParamMaps=param_grid,
                                            # Other params for TrainValidationSplit
                                            )

        # Train the model with hyperparameter tuning
        trainval_split.fit(X_train)

        # Get the best model and best parameters
        best_model = trainval_split.bestModel
        best_params = trainval_split.getBestModelParams()

        print("Best Parameters:", best_params)

        # Evaluate the best model on the validation set (optional)
        predictions = best_model.transform(X_train)
        evaluator = RegressionEvaluator(metricName="rmse")
        rmse = evaluator.evaluate(predictions)
        print("Root Mean Squared Error (RMSE) on validation set:", rmse)

        # Save the best model
        best_model.write.overwrite().save(os.path.join(config.get("root_dir"), config.get("model_name")))
